...
class Usuario extends Maestro {
    ...
    public function __construct($db = "", $modulo = "") {
        ...
        $campos_formulario['email'] = array('tipo' => 'email', 'valor' => '', 'complemento' => "required onchange='validar_email()'");
        ...
        $campos_formulario['clave'] = array('tipo' => 'password', 'valor' => '', 'complemento' => 'required', 'label' => 'Password');
    ...
    function home() {
        ...
        $campos = array('u.' . $this->primary_key . ' as Codigo', 'u.nickname', 'concat(u.nombre," ",u.apellido)  as usuario', 'u.num_documento', 'u.email', 'est.id_estado as Estado');
        $tablas = $this->nombre_tabla . " u, fw_estado est";
        $condicion = "u.estado_id=est.id_estado AND u.interlocutor_id=$id_interlocutor AND u.estado_id <>" . ESTADO_ELIMINADO . ' and u.id_usuario <>' . $id_usuario; //Preguntar la condicion para mostrar usuarios es que el estado no sea eliminado.
