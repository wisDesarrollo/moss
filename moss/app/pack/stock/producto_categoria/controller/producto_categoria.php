...
class ProductoCategoria extends Maestro {
    ...
    function home() {
        ...
        $marcablanca=$this->gn->get_data_loged('marca_blanca');
        $campos = array('pc.'.$this->primary_key." as Codigo",'pc.orden as Orden', 'pc.nombre as nombre','pc.descripcion as Descripcion','est.id_estado as Estado');
        $tablas = $this->nombre_tabla . ' pc, fw_estado est';
        $condicion = "pc.estado_id=est.id_estado AND pc.usuario_id=$id_usuario AND pc.estado_id <>" . self::ESTADO_ELIMINADO. ' AND pc.marca_blanca='.$marcablanca.' ORDER BY pc.orden';
