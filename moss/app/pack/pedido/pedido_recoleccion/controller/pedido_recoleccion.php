...
class pedidoRecoleccion extends Modulo {
    ...
    function traer_pedido() {
        ...
        if (isset($parametros->return) && !$datos_pedido) {
            ...
            if ($datos_pedido) {
                ...
                $pedidos = $this->db->select(array('p.id_producto', 'p.nombre_corto', 'p.descripcion', 'p.precio', 'pi.cantidad'), $tables, 'pi.producto_id=p.id_producto and pi.pedido_id=pe.id_pedido and pe.id_pedido=' . $id_pedido . ' and pe.interlocutor_id=' . $id_interlocutor . ' and pi.estado_id <> 3 and (pe.estado_id=15 or pe.estado_id=16)', 'id_pedido_item', false, false); //pe.estado_id=16 or
                $total = 0;
                if ($pedidos) {
    ...
    function mostrar_menu($opciones = null){
        ...
        $categorias = $this->db->select(array('id_producto_categoria AS id_tab', 'id_producto_categoria', 'pc.nombre', 'fi.icon'), 'producto_categoria pc, fw_icons fi', "pc.icon_id = fi.id_icon AND pc.estado_id= " . ESTADO_ACTIVO . " AND marca_blanca=" . $marcablanca, 'orden'); // ---> para corregir las categorias por cada interlocutor
        $mesa = $this->db->selectOne(
                array('m.nombre', 'm.id_mesa'), 
                'mesa m', 
                ' m.id_mesa= ' . $id_mesa
            );
    ...
    function pintarPedidoContenedor($mesa){
        require_once('app/pack/pedido/pedido_recoleccion/view/pedido_recoleccion.html.php');
        PedidoRecoleccionHTML::pedidoContenedor($mesa);
    }
    ...
    function pintarPedidoItem(){
        ...
        PedidoRecoleccionHTML::pintarPedidoItem($producto);
        //}
    ...
    function confirmar_pedido(){
        ...
        foreach($parametros_array as $key => $value){
            ...
            if ($nombre_campo[2] == 'cantidad') {
                $items[$nombre_campo[3]][$nombre_campo[2]] = $value;
                $items[$nombre_campo[3]]['producto_id'] = $nombre_campo[3];
                $items[$nombre_campo[3]]['pedido_id'] = $id_pedido;
            }
        }
    ...
    function crear_pedido_item($item) {
