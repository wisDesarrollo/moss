...
require_once('app/controlador/php/modulos/operaciones.php');

class Facturar extends Operaciones{
    ...
    public function __construct($db = "", $modulo = "") {
        ...
        $this->transaccion_tipo    = self::VENTA;
        $this->comercio_clase      = self::CLIENTE;
    ...
    function registrar() {
        ...
        require_once 'app/controlador/php/lib/transacciones.class.php';
        ...
        if ($cliente['id_interlocutor']=='') {
            require_once 'fw/controlador/php/modulos/interlocutor.php';
            $interlocutor = new Interlocutor($this->db,'interlocutor');
            $cliente['interlocutor_id'] = $id_interlocutor;
            $cliente['interlocutor_clase_id'] = self::CLIENTE;
       ...
       $factura_numeracion = $transaccion->numeracion_factura($id_interlocutor);
        ...
        if (!$factura_numeracion) {
            $this->nc->set_notificacion('Numeraci&oacute;n de facturas no disponible');
            header('location:?opcion='.$this->modulo);
            return;
        }
    ...
    function ver_factura($id_transaccion = NULL, $cliente = NULL) {
        ...
        $empresa = $this->db->selectone(array('CONCAT(i.nombre, " ", i.apellido) as empresa', 'i.num_documento AS NIT', 'i.telefono'), 'interlocutor i', ' i.id_interlocutor = ' . $id_interlocutor, false, false, false, false);
        ...
        if (isset($transaccion['id_transaccion'])) {
            ...
            $this->tipo_contenido = 'ajax';
            require_once("app/vista/html/ventas.html.php");
