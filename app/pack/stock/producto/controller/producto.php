...
Class Producto extends Maestro {
    ...
    public function __construct($db = "", $modulo = "") {
        ...
        $campos_formulario['producto_categoria_id'] = array('tipo' => 'select', 'valor' => '','tag' => 'select', 'label'=>'Categoria',
                                                            ...
                                                            'condicion' => 'pc.estado_id ='.self::ESTADO_ACTIVO."                                                                             
                                                                    AND pc.usuario_id=$id_usuario "
                                                                    . " AND pc.usuario_id=u.id_usuario "
                                                                    . " AND u.interlocutor_id=i.id_interlocutor "
                                                                    . " AND ic.interlocutor_id=i.id_interlocutor "
                                                                    . " AND ic.marca_blanca= $marcablanca");
        $campos_formulario['iva_incluido'] = array('tipo' => 'select', 'valor' => '', 'complemento' => 'required', 'opciones' => array('S' => 'SI', 'N' => 'NO'), 'tag' => 'select', 'label'=>'IVA Incluido');
        $campos_formulario['iva'] = array('tipo' => 'number', 'valor' => '0', 'complemento' => 'required', 'label' => 'IVA (%)');
    ...
    function home() {
        ...
        $campos = array('pr.' . $this->primary_key . " AS empty",'pc.nombre AS Categoria','CONCAT("'.$imagen.'", pr.id_producto, "'.'\"'.$ima.'", pr.id_producto, "'.'.jpg\"'.$img.'", pr.id_producto, "'.$im.$imagen2.'") AS Imagen','pr.nombre as nombre', 'pc.nombre as Categoria ','CONCAT("$", FORMAT(pr.precio, 0)) AS precio','pr.stock_control AS inventario', 'est.id_estado as Estado');
