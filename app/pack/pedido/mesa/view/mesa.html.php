...
class mesaHTML {
    ...
    static function home($mesas, $tipo_negocio, $modulo = '', $obs = '', $zona = -1) {
        ?>
        <div class="tables_container mesa_zona <?php if($zona != 0){ echo 'vista_mesas';}  ?>" id="zona_<?php echo $zona; ?>">           
            <?php
            if ($mesas) {
                foreach ($mesas as $key => $value) {
                    ?>
                    <div class="mesas_contenedor"  onmouseover="this.style.opacity = 0.7" onmouseout="this.style.opacity = 1" onclick="<?php echo $modulo; ?>_accion(<?php echo $value['id_mesa'] ?>,<?php echo $value['estado_id'] ?>, '<?php echo $modulo; ?>', '<?php echo $value['mesa_id']; ?>');">
                        <div id="mesa_<?php echo $value['id_mesa'] ?>" class="item-mesa">
                            <?php if ($value['estado_id'] == ESTADO_DISPONIBLE) { ?>
                                <div class="mesa-estado">
                                    <img src="fw/themes/<?php echo $tipo_negocio; ?>/img/disponible.png">
                                </div>
                            <?php } elseif ($value['estado_id'] == ESTADO_RESERVADO) { ?>
                                <div class="mesa-estado">
    ...
    static function mesas_contneedor_ini($modulo) {
        ?>
        <div id="area_trabajo_mesas" class="centra-contenido aside_vertical wis_bloque <?php echo $modulo; ?>">
    <?php
    }

    static function mesas_contneedor_fin() {
            ?>
        </div>
                                ...
                                foreach ($pedidos as $pedido) {
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <table>
                                                <?php foreach ($pedidos_detalle[$pedido['id_pedido']] as $k => $item) { ?>
