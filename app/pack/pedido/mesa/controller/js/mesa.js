...
function facturar_accion(id, estado, modulo, param){
...
function removeItemFactura(id, mesa) {
    peticion_ajax('?opcion=facturar&a=set_estado_pedido&type=ajax_part&id=' + id+"&id_mesa="+mesa, '', 'subarea_trabajo', 'mostrar_factura');
}
function mostrar_factura(response){
...
function factura_imprimir(){
    peticion_ajax('?opcion=facturar&a=imprimir&type=ajax', '', '');
}

function removeFacturaItem(id, precio) {
   peticion_ajax('?opcion=facturar&a=remover_factura_item&id_item='+id+'&type=ajax', '', '');
}