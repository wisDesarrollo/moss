...
class Facturar extends Modulo {
 ...
 function pintar_mesas($campos, $tablas, $modulo = '') {
    ...
    /*function pintar_mesas($campos, $tablas, $modulo = '') {
        $id_interlocutor = $this->gn->get_data_loged('id_interlocutor');
        $campos = array('m.id_mesa', 'm.nombre', 'm.mesa_id', 'm.descripcion', 'm.estado_id', 'm.mesero_id', 'm.orden');
        $mesas = $this->db->select($campos, 'mesa m, pedido p',
                'm.estado_id ='.ESTADO_OCUPADO.' '
                . ' AND m.id_mesa = p.mesa_id'
                . ' AND p.estado_id = '.ESTADO_PEDIDO_ENTREGADO
