...
class FacturaHTML {
    ...
    static function home($mesas, $tipo_negocio, $modulo = '', $obs) {
        ...
        <div class="centra-contenido aside_vertical wis_bloque <?php echo $modulo; ?>">
            <div class="tables_container vista_mesas">           
                <?php
                if ($mesas) {
	...
	static function refresh(){ ?>
		<script>
            //modulo_refrescar('facturar', true, 40000, false, 'area_trabajo_mesas');
        </script>
<?php 
	}

    static function pintar_item($items, $id_pedido, $id_mesa) {
        ?>
        <?php foreach ($items[$id_pedido] as $k => $item) { ?>
            ...
            <td>
                ...
                <button onclick="removeItemFactura(<?php echo $item['id_pedido_item'] ?>,<?php echo $id_mesa; ?>)" class="wis_button1 red-button" type="button">X</button>          
            </td>
            </tr>
            <?php
        }
    ...
    static function mostrar_pedido($modulo, $cuenta, $pedidos = array(), $pedidos_detalle = array(), $resumen = array(), $var = 1, $id_transaccion = '', $info_negocio = '', $transa = '', $id_mesa = 0, $descuentos = null) {
        if ($cuenta['id_cuenta']) {
            ?>
            ...
            <form id="pedido_producto_form" action="?opcion=facturar&a=facturar&type=ajax_part" method="post" target="_blank" onsubmit="window.location='?opcion=subopcion&id=17&type=html'">
                <?php if ($cuenta && $pedidos) { ?>
                    ...
                    <div class="vista_pedido">
                        <table>
                            ...
                            foreach ($pedidos as $pedido) {
                                ...
                                <?php
                                self::pintar_item($pedidos_detalle, $pedido['id_pedido'],$id_mesa );
                            }
                            ?>
                        </table>
                    </div>
                    <div class="facturar_resumen" id="factura_resumen_totales">
                        <?php
                        self::factura_totales($resumen['subtotal'], $resumen['servicio'], $resumen['impuesto'], $resumen['total'], "", $descuentos);
                        ?>
                    </div>
                <?php } else { ?>
                ...
                <div class="button-container">
                    <?php if ($var == 1) { ?>
                        <button id="boton-facturar-factura" type="submit" class="opcion-button" onclick="facturaGenerarFactura('pedido_producto_form');">Facturar</button>
    ...
    static function factura_totales($subtotal, $servicio, $impuestos, $total, $check_value = "", $descuentos = null, $id_descuento = 0) {
        ?>
        ...
        <div class="total-pedidos ">
            <p class="letra-total letra-resumen"> 
                Incluir Servicio 
                <input type="checkbox" id="incluir_servicio" name="incluir_servicio" onclick="facturaClickServicioCheck('incluir_servicio', <?php echo $subtotal; ?>)" <?php echo $check_value; ?>>
                $
            </p>
            <input type="number" id="factura_servicio" name="factura_servicio"  onblur="factura_validar_servicio('incluir_servicio')" value="<?php echo $servicio; ?>" <?php if($check_value !== "checked"){echo "disabled";} ?>>
            <input type="hidden" id="factura_servicio_valor" name="factura_servicio_valor" value="<?php echo $servicio; ?>">
        </div>
        <div class="total-pedidos ">
            <p class="letra-total letra-resumen"> Descuento: </p>
                <input type="hidden" id="factura_descuento_porcentaje" name="factura_descuento_porcentaje" value="<?php echo $servicio; ?>">
                <select id="factura_descuento" name="factura_descuento" onchange="factura_validar_servicio('incluir_servicio')">
                    <option value="0">Sin Descuento</option>
                    <?php foreach ($descuentos as $descuento) { ?>
                    <option value="<?php echo $descuento['id_descuento']; ?>" <?php if($id_descuento == $descuento['id_descuento']){ echo "selected";} ?>>
                        <?php echo $descuento['descuento']. "% ".$descuento['nombre']; ?>
                    </option>
                    <?php } ?>
                </select>
        </div>
