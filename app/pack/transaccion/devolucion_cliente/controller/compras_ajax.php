...
$columns = array(
    ...
    array('db' => 'p.id_producto', 'dt' => 6, 'field' => 'input', 'as' => 'input',
        'formatter' => function( $d, $row ) {
            return '<input id=' . $d . '_add_cant class="add_cant" type="text" value="1">';
        }),
    array('db' => 'p.id_producto', 'dt' => 7, 'field' => 'boton', 'as' => 'boton',
        'formatter' => function( $d, $row ) {
            $onclick = 'onclick="TrxAddProduct(\'' . $d . '\',\'compras\');"';
            return '<button type="button" class="add_button" ' . $onclick . ' title="adicionar" onfocus="this.blur();">+</button>';
        }),
);

$joinQuery = " FROM producto p 
            JOIN producto_stock ps ON ps.producto_id = p.id_producto";
$extraWhere = "p.interlocutor_id='" . $datos['id_interlocutor'] . "' and p.estado_id <> 3 AND p.stock_control = 'S'";
$groupBy = "";
