...
class Maestro extends Modulo {
    ...
    function iniciar_maestro_slave($campos, $tablas, $condicion, $titulo_pdf = '') {
        ...
        $this->pintar_herramientas($this->modulo, $this->herramientas, 'validar_herramienta_slave');
        ...
        $accion = array('radio_edicion_slave' => 'formulario_edicion');
        $this->pintar_tabla($campos, $tablas, $condicion, $this->modulo, $accion, $function, $titulo_pdf, 'tabla_' . $this->modulo);
    }
    ...
    function cambiar_estado_master() {
        ...
        $id = $parametros->id;
        $respuesta_update = $this->db->update(array('estado_id' => $estado), $tabla, 'id_' . $modulo . '=' . $id);
        header('location:?opcion=' . $modulo . '&type=ajax');
    }
