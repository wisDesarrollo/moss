...
class Modulo {
	...
	function pintar_herramientas($modulo, $herramientas = array('eliminar', 'editar', 'nuevo'), $function = 'validar_herramienta') {
	...
	function pintar_tabla($campos, $tabla, $condicion, $modulo = '', $accion = '', $function = array(), $titulo_pdf = '', $id_tabla = 'example', $select = "select", $checkbox = array(), $nombre_tabla = '') {
		...
		if (intval($filas) > 5000) {
			...
			require_once 'vista/html/datatable.html.php';
	...
	function pintar_mesas($campos, $tablas, $zona, $modulo = '') {
            ...
            $mesas = $this->db->select($campos, $tablas, 'm.estado_id = est.id_estado AND  m.estado_id <> 3 AND m.mesa_id = '.$zona.' AND m.interlocutor_id=' . $id_interlocutor, 'm.id_mesa', false, false);
